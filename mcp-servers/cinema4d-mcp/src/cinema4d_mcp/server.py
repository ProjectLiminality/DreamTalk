...
async def describe_hierarchy(
    ...
    """
    Describe scene hierarchy with semantic meaning for AI assistance.

    Returns a structured description of the Cinema 4D scene that understands
    DreamTalk object semantics (CustomObject, LineObject, SolidObject).
...
async def inspect_object(
    ...
    Returns detailed information about the object including transform,
    userdata parameters, tags, materials, and hierarchy info.
...
@mcp.tool()
async def inspect_materials(
    ctx: Context = None,
) -> str:
    """
    Describe all materials in the scene.

    Returns information about each material including color, properties,
    and which objects are using them.
    """
    async with c4d_connection_context() as connection:
        if not connection.connected:
            return "❌ Not connected to Cinema 4D"
...
async def inspect_animation(
    ...
    Returns information about animated objects, their keyframes,
    and what parameters are being animated.
    ...
    async with c4d_connection_context() as connection:
        ...
        command = {"command": "inspect_animation"}
...
async def validate_scene(
    ...
    Checks for common issues like missing materials, unused materials,
    objects at unexpected positions, and animation state.
    """
    async with c4d_connection_context() as connection:
        if not connection.connected:
            return "❌ Not connected to Cinema 4D"
