...
class C4DSocketServer(threading.Thread):
            ...
            while self.running:
                ...
                while "\n" in buffer:
                    ...
                    try:
                        ...
                        elif command_type == "describe_hierarchy":
                            response = self.handle_describe_hierarchy(command)
                        elif command_type == "inspect_object":
                            response = self.handle_inspect_object(command)
                        elif command_type == "inspect_materials":
                            response = self.handle_inspect_materials(command)
                        elif command_type == "inspect_animation":
    ...
    def handle_describe_hierarchy(self, command):
        """
        Handle describe_hierarchy command - returns semantic scene description.

        Tries to use DreamTalk introspection module if available, falls back
        to basic C4D object listing otherwise.
        """
        doc = c4d.documents.GetActiveDocument()
        format_type = command.get("format", "markdown")

        try:
            # Try to use DreamTalk introspection if available
            from DreamTalk.introspection import describe_hierarchy, format_for_ai
            result = describe_hierarchy(doc)
            description = format_for_ai(result, format_type)
            self.log("[C4D] Used DreamTalk introspection for hierarchy description")
        except ImportError:
            # Fallback to basic description
            description = self._basic_hierarchy_description(doc, format_type)
            self.log("[C4D] Used fallback hierarchy description (DreamTalk not available)")
        except Exception as e:
            self.log(f"[**ERROR**] Error in describe_hierarchy: {str(e)}")
            return {"error": f"Failed to describe hierarchy: {str(e)}"}

        return {"description": description}

    def _basic_hierarchy_description(self, doc, format_type="markdown"):
        """
        Fallback hierarchy description when DreamTalk is not available.

        Returns basic C4D object listing.
        """
        lines = []
        lines.append(f"# Scene: {doc.GetDocumentName() or 'Untitled'}")
        lines.append("")

        def describe_obj(obj, depth=0):
            indent = "  " * depth
            prefix = "└── " if depth > 0 else ""
            pos = obj.GetAbsPos()
            name = obj.GetName()
            obj_type = obj.GetTypeName()
            lines.append(f"{indent}{prefix}{name} ({obj_type}) @ ({pos.x:.1f}, {pos.y:.1f}, {pos.z:.1f})")

            child = obj.GetDown()
            while child:
                describe_obj(child, depth + 1)
                child = child.GetNext()
        ...
        lines.insert(1, f"**{count} root object(s)**")
        lines.insert(2, "")
    ...
    def handle_inspect_object(self, command):
        """Handle inspect_object command - deep dive into single object."""
        ...
        try:
            from DreamTalk.introspection import inspect_object
            from DreamTalk.introspection.formatters import format_inspect_object
            result = inspect_object(object_name, doc)
            description = format_inspect_object(result)
            self.log(f"[C4D] Inspected object: {object_name}")
        except ImportError:
            description = f"DreamTalk introspection not available. Object '{object_name}' exists but cannot be inspected."
            self.log("[C4D] DreamTalk introspection not available")
        except Exception as e:
            return {"error": f"Failed to inspect object: {str(e)}"}

        return {"description": description}

    def handle_inspect_materials(self, command):
        """Handle inspect_materials command - describe all materials."""
        doc = c4d.documents.GetActiveDocument()

        try:
            from DreamTalk.introspection import inspect_materials
            from DreamTalk.introspection.formatters import format_inspect_materials
            result = inspect_materials(doc)
            description = format_inspect_materials(result)
            self.log("[C4D] Inspected materials")
        except ImportError:
            # Fallback
            ...
            while mat:
                ...
                count += 1
                mat = mat.GetNext()
            description = f"# Materials ({count})\n\n" + "\n".join([f"- {n}" for n in names])
            self.log("[C4D] Used fallback materials inspection")
        except Exception as e:
            return {"error": f"Failed to inspect materials: {str(e)}"}

        return {"description": description}

    def handle_inspect_animation(self, command):
        """Handle inspect_animation command - describe animation keyframes."""
        doc = c4d.documents.GetActiveDocument()
        start_frame = command.get("start_frame")
        end_frame = command.get("end_frame")

        try:
            from DreamTalk.introspection import inspect_animation
            from DreamTalk.introspection.formatters import format_inspect_animation
            result = inspect_animation(start_frame, end_frame, doc)
            description = format_inspect_animation(result)
            self.log("[C4D] Inspected animation")
        except ImportError:
            fps = doc.GetFps()
            doc_start = doc.GetMinTime().GetFrame(fps)
            doc_end = doc.GetMaxTime().GetFrame(fps)
            description = f"# Animation\n\nFrame range: {doc_start} - {doc_end}\nFPS: {fps}\n\n(DreamTalk introspection not available for detailed analysis)"
            self.log("[C4D] Used fallback animation inspection")
        except Exception as e:
            return {"error": f"Failed to inspect animation: {str(e)}"}

        return {"description": description}

    def handle_validate_scene(self, command):
        """Handle validate_scene command - pre-render sanity checks."""
        doc = c4d.documents.GetActiveDocument()

        try:
            from DreamTalk.introspection import validate_scene
            from DreamTalk.introspection.formatters import format_validate_scene
            result = validate_scene(doc)
            description = format_validate_scene(result)
            self.log("[C4D] Validated scene")
        except ImportError:
