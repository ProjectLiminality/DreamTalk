...
const frames = pos.slice(1, -1).map(Number)
...
try {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await page.evaluate((t) => (window as any).__dt.setT(t), at)
  ...
  for (const idx of frames) {
    ...
    try {
      const m = JSON.parse(line) as Omit<FrameReport, "idx" | "videoSec">
      reports.push({ idx, videoSec: (idx - 1) / 5, ...m })
      console.log(
